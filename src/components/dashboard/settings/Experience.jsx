...
export default class Experience extends React.Component {
  ...
  onAddWork(work = {}, e) {
    ...
    openModal(
      <WorkForm ProfileActions={ProfileActions} work={work} isSaved={{}} isSaving={{}} errors={errors} />,
      'Add work experience'
    ).then(data => {
      if (work && work.id) {
  ...
  onAddEducation(education = {}, e) {
    ...
    openModal(
      <EducationForm ProfileActions={ProfileActions} education={education} isSaved={{}} isSaving={{}} errors={errors} />,
      'Add education'
    ).then(data => {
  ...
  render() {
    ...
    return (
      <div>
        {this.props.isSaved.profile ? <Success message="Profile saved successfully" /> : null}

        <form onSubmit={this.onSave}>
          <div className="row">
            <div className="col-sm-8">
              {errors.profile && errors.profile.bio ? <FieldError message={errors.profile.bio} /> : null}
              <FormGroup>
              ...
              {[
                ...
                return (
                  <FormGroup key={categoryId}>
                    ...
                    <SkillSelector
                      filter={{ filter: null }}
                      onChange={this.onSkillChange.bind(this, categoryId)}
                      selected={this.filterSkills(categoryId) || []}
          ...
          <FormGroup>
            <div className="clearfix">
              <Button type="submit" className="float-right" disabled={this.props.isSaving.profile}>
                Save
              </Button>
            </div>
          </FormGroup>
          ...
          <div className="row">
            <div className="col-sm-8">
              <FormGroup>
                ...
                <div>
                  <Button onClick={this.onAddWork.bind(this, {})}>add entry</Button>
                </div>
              </FormGroup>
            </div>
          </div>
          ...
          <div className="row">
            <div className="col-sm-8">
              {isSaving.work ? <Progress message="Saving ..." /> : null}

              {isSaved.work ? <Success message="Work Experience saved successfully" /> : null}

              {user.work && user.work.length
                ? user.work.map(work => {
                    return (
                      <div className="card work-education-wrapper">
                        <div className="card-body">
                          ...
                          <div>
                            Period: {work.start_month_display}/{work.start_year} -{' '}
                            {work.end_year ? `${work.end_month_display}/${work.end_year}` : 'Present'}
                          </div>
                          <br />
                          <p>{work.details}</p>
                          <Button onClick={this.onAddWork.bind(this, work)}>
          ...
          <div className="row">
            <div className="col-sm-8">
              <FormGroup>
                ...
                <div>
                  <Button className="btn btn-primary" onClick={this.onAddEducation.bind(this, {})}>
                    add entry
                  </Button>
                </div>
              </FormGroup>
          ...
          <div className="row">
            <div className="col-sm-8">
              {isSaving.education ? <Progress message="Saving ..." /> : null}

              {isSaved.education ? <Success message="Education saved successfully" /> : null}
                    ...
                    return (
                      <div className="card work-education-wrapper">
                        <div className="card-body">
                          ...
                          <div>
                            Period: {education.start_month_display}/{education.start_year} -{' '}
                            {education.end_year ? `${education.end_month_display}/${education.end_year}` : 'Present'}
                          </div>
                          <br />
                          <p>{education.details}</p>
                          <Button onClick={this.onAddEducation.bind(this, education)}>
                            <Icon name="pencil2" /> Edit
                          </Button>
                        </div>
                      </div>
